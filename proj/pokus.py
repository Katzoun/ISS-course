...
USE_FILE = True # if True, use file-based storage of fingerprints, 
...
print(f"USE FILE-BASED STORAGE = {USE_FILE}" )
...
def build_fingerprint_database(known_signals, use_file: Optional[bool] = False, filename: Optional[str] ="fingerprint.pkl"):
...
def recognize_signal(signal, fingerprint_db):
    """Recognize the song from the given signal using the fingerprint database."""
    hashes = generate_fingerprint(signal)
    ...
    for hash_value, time_offset in hashes:
        if hash_value in fingerprint_db:
            ...
            for entry_time_offset, song_index in entries:
                matches[song_index] += 1
    
    return matches
    # if matches:
    #     recognized_song = max(matches, key=matches.get)
    #     print("Top 5 matches:")
    #     sorted_matches = sorted(matches.items(), key=lambda item: item[1], reverse=True)
    #     print("Top matches:")
    #     for song_index, match_count in sorted_matches[:5]:
    #         print(f"Song index: {song_index}, Matching hashes: {match_count}")

    #     print(f"Recognized song index: {recognized_song} with {matches[recognized_song]} matching hashes")
    #     return recognized_song
    # else:
    #     print("No matching song found.")
    #     return None
    
def compute_similarity_matrix(valid_signals, fingerprint_db , N_valid: int, N_known: int):
    """Compute similarity matrix between valid and known signals."""
    similarities = np.zeros((N_valid, N_known), dtype=int)

    for valid_id in tqdm(range(N_valid) , desc="Computing similarities", unit="validation signal"):
        ...
        for song_index, match_count in recognized_song.items():
            similarities[valid_id, song_index] = match_count
...
if __name__ == "__main__":
    ...
    # PHASE4
    fingerprint_db = build_fingerprint_database(known_signals, use_file=USE_FILE, filename=FILE_NAME)
